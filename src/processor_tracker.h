...
class ProcessorTracker : public ProcessorBase
{
    protected:
        ...
        int max_new_features_; ///< max features alowed to detect in one iteration. -1 = no limit

    public:
        ProcessorTracker(ProcessorType _tp, const std::string& _type, const int _max_new_features, const Scalar& _time_tolerance);
