                                                         ...
                                                         FeatureMatchMap& _feature_correspondences)
{
    ...
    for (auto feat_in_ptr : _feature_list_in)
    {
        ...
        {
            _feature_list_out.push_back(new FeatureBase(FEAT_POINT_IMAGE, "POINT IMAGE", feat_in_ptr->getMeasurement(), feat_in_ptr->getMeasurementCovariance()));
...
unsigned int ProcessorTrackerFeatureDummy::detectNewFeatures(const unsigned int& _max_features)
{
    ...
    for (unsigned int i = 1; i <= 5; i++)
    {
        ...
        new_features_last_.push_back(
                new FeatureBase(FEAT_POINT_IMAGE, "POINT IMAGE", n_feature_* Eigen::Vector1s::Ones(), Eigen::MatrixXs::Ones(1, 1)));
