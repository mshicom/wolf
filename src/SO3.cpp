...
ExpmapFunctor::ExpmapFunctor(const Eigen::Vector3s& omega, bool nearZeroApprox)
...
ExpmapFunctor::ExpmapFunctor(const Eigen::Vector3s& axis, double angle, bool nearZeroApprox)
    : theta2(angle * angle) {
...
Eigen::Vector3s DexpFunctor::applyDexp(const Eigen::Vector3s& v, OptionalJacobian<3, 3> H1,
...
Eigen::Vector3s DexpFunctor::applyInvDexp(const Eigen::Vector3s& v, OptionalJacobian<3, 3> H1,
                                  OptionalJacobian<3, 3> H2) const {
  const Eigen::Matrix3s invDexp = dexp_.transpose();
  const Eigen::Vector3s c = invDexp * v;
  if (H1) {
    Eigen::Matrix3s D_dexpv_omega;
...
/* ************************************************************************* */
SO3 SO3::AxisAngle(const Eigen::Vector3s& axis, double theta) {
  return so3::ExpmapFunctor(axis, theta).expmap();
}

SO3 SO3::Expmap(const Eigen::Vector3s& omega, ChartJacobian H) {
  if (H) {
...
Eigen::Matrix3s SO3::ExpmapDerivative(const Eigen::Vector3s& omega) {
  return so3::DexpFunctor(omega).dexp();
}

/* ************************************************************************* */
Eigen::Vector3s SO3::Logmap(const SO3& R, ChartJacobian H) {
  using std::sqrt;
  using std::sin;
  ...
  Eigen::Vector3s omega;

  // when trace == -1, i.e., when theta = +-pi, +-3pi, +-5pi, etc.
  // we do something special
  if (std::abs(tr + 1.0) < 1e-10) {
    if (std::abs(R33 + 1.0) > 1e-10)
      omega = (M_PI / sqrt(2.0 + 2.0 * R33)) * Eigen::Vector3s(R13, R23, 1.0 + R33);
    else if (std::abs(R22 + 1.0) > 1e-10)
      omega = (M_PI / sqrt(2.0 + 2.0 * R22)) * Eigen::Vector3s(R12, 1.0 + R22, R32);
    else
      // if(std::abs(R.r1_.x()+1.0) > 1e-10)  This is implicit
      omega = (M_PI / sqrt(2.0 + 2.0 * R11)) * Eigen::Vector3s(1.0 + R11, R21, R31);
  } else {
    double magnitude;
    ...
    omega = magnitude * Eigen::Vector3s(R32 - R23, R13 - R31, R21 - R12);
  }
...
/* ************************************************************************* */
Eigen::Matrix3s SO3::LogmapDerivative(const Eigen::Vector3s& omega) {
  using std::cos;
  using std::sin;
