...
/// A helper class that implements the traits interface for lie groups.
...
template<class Class>
inline Eigen::VectorXd logmap_default(const Class& l0, const Class& lp) {
  return Class::Logmap(l0.between(lp));
}

/** Exponential map centered at l0, s.t. exp(t,d) = t*exp(d) */
template<class Class>
inline Class expmap_default(const Class& t, const Eigen::VectorXd& d) {
 ...
 */
template <class T> Eigen::MatrixXd wedge(const Eigen::VectorXd& x);
 ...
 */
template <class T>
T expm(const Eigen::VectorXd& x, int K=7) {
  Eigen::MatrixXd xhat = wedge<T>(x);
