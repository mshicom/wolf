        ...
        while (std::getline(offLineFile_, buffer))
        {
            ...
            else if (buffer.at(0) == 'E')
            {
                ...
                if (edge_new <= MAX_VERTEX+1 && edge_old <= MAX_VERTEX+1 )
                {
                    ...
                    FeatureBase* feature_ptr_ceres_diff = new FeatureBase(FEAT_FIX, "FIX", edge_vector, edge_information.inverse());
                    FeatureBase* feature_ptr_wolf_diff = new FeatureBase(FEAT_FIX, "FIX", edge_vector, edge_information.inverse());
