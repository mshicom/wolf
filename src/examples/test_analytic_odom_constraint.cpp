        ...
        while (std::getline(offLineFile_, buffer))
        {
            ...
            else if (buffer.at(0) == 'E')
            {
                ...
                if (edge_new <= MAX_VERTEX+1 && edge_old <= MAX_VERTEX+1 )
                {
                    ...
                    FeatureBase* feature_ptr_autodiff = new FeatureBase(FEAT_FIX, "FIX", edge_vector, edge_information.inverse());
                    ...
                    // add capture, feature and constraint to problem
                    FeatureBase* feature_ptr_analytic = new FeatureBase(FEAT_FIX, "FIX", edge_vector, edge_information.inverse());
